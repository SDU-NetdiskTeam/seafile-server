...
func Get(id string) *Repo {
	...
	if commit.Standardized == "true" {
		repo.IsStandardized = true
		repo.Hint = commit.Hint
	}
	if commit.Sized == "true" {
		repo.IsSized = true
		repo.Size = commit.Size
		repo.Unit = commit.Unit
	}
...
func RepoToCommit(repo *Repo, commit *commitmgr.Commit) {
	...
	if repo.IsStandardized {
		commit.Standardized = "true"
		commit.Hint = repo.Hint
	} else {
		commit.Standardized = "false"
	}
	if repo.IsSized {
		commit.Sized = "true"
		commit.Size = repo.Size
		commit.Unit = repo.Unit
	} else {
		commit.Sized = "false"
	}
...
func SetRepoCommitToDb(repoID, repoName string, updateTime int64, version int, isEncrypted string, isStandardized string, isSized string, lastModifier string) error {
	var exists int
	var encrypted int
	var standardized int
	var sized int
	...
	if exists == 1 {
		sqlStr := "UPDATE RepoInfo SET name=?, update_time=?, version=?, is_encrypted=?, " +
			"is_standardized=?, is_sized=?, last_modifier=? WHERE repo_id=?"
		if _, err := seafileDB.Exec(sqlStr, repoName, updateTime, version, encrypted, lastModifier, repoID); err != nil {
			return err
		}
	} else {
		sqlStr := "INSERT INTO RepoInfo (repo_id, name, update_time, version, is_encrypted, is_standardized, is_sized, last_modifier) " +
			"VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
